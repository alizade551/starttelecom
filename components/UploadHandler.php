...
class UploadHandler
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->response = array();
        $this->options = array(
            'script_url' => '/photo-upload',
            'upload_dir' => Yii::getAlias("@app").'/web/uploads/user_photos/',
            'upload_url' => '/uploads/user_photos/',
...
// $type = strtolower(substr(strrchr($file_name, '.'), 1));
    ...
    protected function rotate($file,$rotate){
        if ($rotate == 'right') {
            $degrees = 90;
        }else{
             $degrees = -90;
        ...
        $main = Yii::getAlias('@app').'/web/uploads/user_photos/'. $file;
        $filename = Yii::getAlias('@app').'/web/uploads/user_photos/medium/' . $file;
        $thumbnail =Yii::getAlias('@app'). '/web/uploads/user_photos/thumbnail/' . $file;

        switch (exif_imagetype($main)) {
        case IMAGETYPE_PNG:
            header('Content-type: image/png');
            $source = imagecreatefrompng($filename);
            $rotate = imagerotate($source, $degrees, 0);
            imagepng($rotate, realpath($filename));

            $source_main = imagecreatefrompng($main);
            $rotate_main = imagerotate($source_main, $degrees, 0);
            imagepng($rotate_main, realpath($main));
            
            Image::resize($filename, $this->options['image_versions']['thumbnail']['max_width'], $this->options['image_versions']['thumbnail']['max_height'])->save($thumbnail, ['quality' => 100]);
        break;
        default:
            header('Content-type: image/jpeg');
            $source = imagecreatefromjpeg($filename);
            $rotate = imagerotate($source, $degrees, 0);
            imagejpeg($rotate, realpath($filename));

            $source_main = imagecreatefromjpeg($main);
            $rotate_main = imagerotate($source_main, $degrees, 0);
            imagejpeg($rotate_main, realpath($main));
    ...
    public function get($print_response = true) {
        if ($this->get_query_param("rotate")) {
            echo $this->rotate($_GET["file"],$_GET["rotate"]);
           return;
        }
