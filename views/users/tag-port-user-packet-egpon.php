...
<div style="max-width:600px">
	<h5 style="margin-bottom:15px"><?=Yii::t(
		'app',
	'Inet login : {login} - {device} device {ponPort} pon-port number  {box_number} box information',
	[
		'device'=>$checkEgponPort->egonBox->device->name,
		'ponPort'=>$checkEgponPort->egonBox->pon_port_number,
		'box_number'=>$checkEgponPort->egonBox->box_number,
		'login'=>$model->usersInet->login
	]
	) ?></h5>
	<div class="port-container">
		<?php $count = 0; ?>
		<?php foreach ($egponBoxPorts as $key => $port): ?>
			<?php $count++ ?>
			<div class="port">
				<?php if ( $checkEgponPort['id'] == $port['id'] ): ?>
				<a href="javascript::void(0)" class="tag_port_confrim" 
					...
					data-port_id="<?=$checkEgponPort['id'] ?>" 
					data-device_type="<?=$checkEgponPort->egonBox->device->type ?>">
				<?php else: ?>
